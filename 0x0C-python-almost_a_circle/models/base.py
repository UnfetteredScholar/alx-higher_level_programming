...
class Base:
    """Base class of all objects in package"""
    __nb_objects = 0
    ...
    def to_json_string(list_dictionaries):
        ...
        Returns:
            JSON string representation of list_dictionaries or '[]'
            if list_dictionaties is None or empty
        """
    ...
    def load_from_file(cls):
        ...
        if os.path.exists(f"{cls.__name__}.json"):
            with open(f"{cls.__name__}.json", 'r', encoding='utf-8') as f:
                json_string = f.read()
                return [cls.create(**jdict)
                        for jdict in Base.from_json_string(json_string)]
        else:
            return []
